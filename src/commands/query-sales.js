...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("address").setRequired(true).setDescription("List of collection addresses to filter by")
    ),
  async execute(interaction) {
    ...
    tokenDetails.sales.nodes.forEach((sale) => {
      sales.push({
        ...
        description: sale.token.description ? sale.token.description : "Unknown",
        collection_name: sale.token.tokenContract.name ? sale.token.tokenContract.name : "Unknown",
        collection_address: sale.token.tokenContract.collectionAddress ? sale.token.tokenContract.collectionAddress : "Unknown",
        symbol: sale.token.tokenContract.symbol ? sale.token.tokenContract.symbol : "Unknown",
        network: sale.token.tokenContract.network ? sale.token.tokenContract.network : "Unknown",
        mint_price: sale.token.mintInfo.price.usdcPrice.decimal ? sale.token.mintInfo.price.usdcPrice.decimal : "Unknown",
        thumbnail_url: sale.token.image.mediaEncoding.thumbnail ? sale.token.image.mediaEncoding.thumbnail : "https://zora.co/assets/og-image.png",
        sale_contract_address: sale.sale.saleContractAddress ? sale.sale.saleContractAddress : "Unknown",
        buyer_address: sale.sale.buyerAddress ? sale.sale.buyerAddress : "Unknown",
        seller_address: sale.sale.sellerAddress ? sale.sale.sellerAddress : "Unknown",
        sale_price: sale.sale.price.usdcPrice.decimal ? sale.sale.price.usdcPrice.decimal : "Unknown",
      })
    })
    ...
    sales.forEach((token) => {
      let embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: 'MINT PRICE', value: `${token.mint_price} USDC`, inline: true },
          { name: 'SALE PRICE', value: `${token.sale_price} USDC`, inline: true },
