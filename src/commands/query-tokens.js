    ...
    try {
      ...
      tokenDetails.tokens.nodes.forEach((token) => {
        tokens.push({
          ...
          mint_price: token.token.mintInfo.price.nativePrice.currency.decimals,
          mint_unit: token.token.mintInfo.price.nativePrice.currency.name,
      ...
      tokens.forEach((token) => {
        let embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'MINT PRICE', value: `${token.mint_price} ${token.mint_unit}` },
          )
        embeds.push(embed)
      })
